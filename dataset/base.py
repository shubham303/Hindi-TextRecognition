...
class BaseDataset(Dataset):
    ...
    def filter(self, label, return_len=False):
        if not self.data_filter:
            if not return_len:
                return False, label
            return False, label, len(label)
        ...
        l = re.sub(out_of_char, '', label.lower())  # remove labels which contain character not in self.character

        # filter whose label larger than batch_max_length
        if len(l) != len(label) or len(label) == 0 or len(
                label) > self.batch_max_length or not self.abfn_filter.is_valid_label(label) or len(
            self.abfn_filter.tokenize(label)) > self.max_tokens:
            if not return_len:
                return True, label
            return True, label, len(label)
